(() => {
    ...
    (() => {
        ...
        products.forEach((el => {
            ...
            el.closest(".product").setAttribute("data-pid", randomID());
        }));
        window.addEventListener("click", (function(event) {
            let counter;
            if ("plus" === event.target.dataset.action || "minus" === event.target.dataset.action || "delete" === event.target.dataset.action) {
                const counterWrapper = event.target.closest(".cart-product");
                counter = counterWrapper.querySelector("[data-quantity-value]");
            }
            if ("plus" === event.target.dataset.action) counter.innerText = ++counter.innerText;
            if ("delete" === event.target.dataset.action) {
                counter.value = 0;
                calcCartPriceAndDelivery();
                event.target.closest(".cart-content__item").remove();
            }
            if ("minus" === event.target.dataset.action) if (event.target.closest(".cart-list") && 0 == counter.value) {
                calcCartPriceAndDelivery();
                event.target.closest(".cart-content__item").remove();
                toggleCartStatus();
            }
            if (event.target.hasAttribute("data-action") && event.target.closest(".cart-list")) calcCartPriceAndDelivery();
        }));
        const cartWrapper = document.querySelector(".cart-list");
        window.addEventListener("click", (function(event) {
            if (event.target.hasAttribute("data-cart")) {
                const targetElement = event.target;
                let id = targetElement.closest(".product").dataset.id;
                let product = document.querySelector(`[data-id="${id}"]`);
                product.querySelector(".item__img-prod");
                const card = event.target.closest(".product");
                const productInfo = {
                    id: card.dataset.id,
                    imgSrc: card.querySelector(".item__img-prod").getAttribute("src"),
                    title: card.querySelector(".item-menu__title").innerText,
                    price: card.querySelector(".price").innerText
                };
                productInfo.price = productInfo.price.replace(/[^0-9,.]/g, " ");
                const itemInCart = cartWrapper.querySelector(`[data-id="${productInfo.id}"]`);
                if (itemInCart) {
                    const counterElement = itemInCart.querySelector("[data-quantity-value]");
                    counterElement.value++;
                } else {
                    const cartItemHTML = `\n\t\t\t<li  class="cart-content__item">\n\t\t\t<article class="cart-content__product cart-product" data-id="${productInfo.id}">\n\t\t\t\t\n\t\t\t\t<img src="${productInfo.imgSrc}" class="cart-product__img" alt="${productInfo.title}" >\n\t\t\t\t\n\t\t\t\t<div class="cart-product__text">\n\t\t\t\t\t<h4 class="cart-product__title">${productInfo.title}</h4>\n\t\t\t\t\t<span class="cart-product__price">${productInfo.price} USD</span>\n\t\t\t\t\n\t\n\t\t\t\t\t<div data-quantity class="quantity counter-wrapper">\n\t\t\t\t\t<button data-quantity-minus data-action="minus" type="button" class="quantity__button quantity__button_minus"></button>\n\t\t\t\t\t<div class="quantity__input">\n\t\t\t\t\t\t<input data-quantity-min="-0.1" data-quantity-value autocomplete="off" type="text" name="form[]" value="1" >\n\t\t\t\t\t</div>\n\t\t\t\t\t<button data-quantity-plus data-action="plus" type="button" class="quantity__button quantity__button_plus"></button>\n\t\t\t\t</div>\n\t\t\t\t</div>\n\t\t\t\t<button data-action="delete" class="cart-product__delete" aria-label="Delete item">x</button>\n\t\t\t</article>\n\t\t\t</li>\n\t\n\t`;
        ...
        function calcCartPriceAndDelivery() {
            ...
            let itemQty = cartWrapper.querySelectorAll("[data-quantity-value]");
            priceTotal = 0;
            priceElements.forEach((function(item) {
                const amountEl = item.closest(".cart-content__item").querySelector("[data-quantity-value]");
                priceTotal += parseFloat(item.innerText) * amountEl.value;
            }));
            let itemTotal = 0;
            itemQty.forEach((function(e) {
                itemTotal += 1 * e.value;
            }));
            const printQuantity = () => {
                cartQuantity.innerText = `${itemTotal} `;
                cartQuantity.innerText > 0 ? cartQuantity.classList.add("active") : cartQuantity.classList.remove("active");
            };
            totalPriceEl.innerText = `${priceTotal.toFixed(2)} USD`;
            printQuantity();
        }
        ...
        productsBtn.forEach((el => {
            el.addEventListener("click", (e => {
                const targetElement = e.target;
                let pid = targetElement.closest(".product").dataset.pid;
                addToCart(targetElement, pid);
            }));
        }));
        function addToCart(targetElement, pid) {
            targetElement.classList.add("_fly");
            const cart = document.querySelector(".cart-header__icon");
            const product = document.querySelector(`[data-pid="${pid}"]`);
            const image = product.querySelector(".item__img-prod");
            const productImageFly = image.cloneNode();
            const productImageFlyWidth = image.offsetWidth;
            const productImageFlyHeight = image.offsetHeight;
            const productImageFlyTop = image.getBoundingClientRect().top;
            const productImageFlyLeft = image.getBoundingClientRect().left;
        ...
        const modalBtn = document.querySelector(".cart-content__btn");
        const modalQty = document.querySelector(".order-modal__quantity span");
        const modalTotalPrice = document.querySelector(".order-modal__summ span");
        const orderProductsList = document.querySelector(".order-modal__list");
        console.log(modalQty.textContent);
        modalBtn.addEventListener("click", (function(event) {
            if (modalBtn) {
                let cartQuantity = document.querySelector(".cart__quantity");
                let totalPriceEl = document.querySelector(".fullprice");
                modalTotalPrice.textContent = totalPriceEl.textContent;
                modalQty.textContent = cartQuantity.textContent;
                ...
                cartProducts.forEach((product => {
                    ...
                    const productQty = product.querySelector("[data-quantity-value]").value;
